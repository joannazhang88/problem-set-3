...
When shrinkage is 0.01, we get a training MSE of 360 and a test MSE of 392. The test MSE we get from a shrinkage of 0.01 is not optimal, since the test MSE reaches its lowest point at a shrinkage of about 0.003. 
...
Of all the methods, linear regression produces the smallest test MSE, which implies that the linear regression model could be the best model. But note that the MSEs of the models may change once we reset the random process. It is possible that once we reset the seeds, the linear regression model no longer generates the lowest MSE.
...
tune_c <- tune(svm, Purchase ~ ., data = trainOJ, kernel = "linear",
               ranges = list(cost = c(0.01, 0.1, 1, 5, 10, 100, 500, 1000)))
summary(tune_c)
...
The train error and the test error using the optimal cost are 17.8% and 14.4%, respectively. Compared to the previous classifiers, the optimal classifier produces smaller test and training errors. We have improved the test error by about 6%. And compared to the classifier with cost = 0.01, the optimal classifier has more Citrus Hill misclassified as Minute Maid, but less Minute Maid misclassified as Citrus Hill.
